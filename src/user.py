...
class User:
    ...
    def get_word_statistics(self):
        ...
        most_used_word = (
            self.pd_word_df["Word"].value_counts().reset_index()["index"].tolist()[0]
        )
        most_used_double_word = (
            self.pd_bigrams_df["Word"].value_counts().reset_index()["index"].tolist()[0]
        )
        most_used_triple_word = (
            self.pd_trigrams_df["Word"]
            .value_counts()
            .reset_index()["index"]
            .tolist()[0]
        )
    ...
    def get_emoji_statistics(self):
        ...
        most_used_emoji = (
            self.pd_emoji_df["Emoji"].value_counts().reset_index()["index"].tolist()[0]
        )
    ...
    def get_response_time(self, data=None):
        ...
        response_obj = Response(data=data, logger=self.logger)
        response_obj.group_the_data().create_response_time()
    ...
    def get_top_k_emojis(self, k=20, normalize=True):
        ...
        return (
            ...
            .head(k)
            .reset_index()
            .rename(columns={"index": "Emoji", "Emoji": "Count"})
        )
    ...
    def get_userwise_monthly_response_time(self, data):
        ...
        response_obj = Response(data=data, logger=self.logger)
        response_obj.group_the_data().create_response_time()
        response_obj.grouped_data["Month"] = response_obj.grouped_data[
            "Timestamp"
        ].dt.strftime("(%Y) %m")
        userwise_monthly_response_time = (
            response_obj.grouped_data.groupby(["Month", "User"])["Response (Min)"]
            .agg(User.harmonic_mean)
            .reset_index()
        )
        userwise_monthly_response_time.loc[
            userwise_monthly_response_time["Response (Min)"] > 60, "Response (Min)"
        ] = -1
